#adapt the clean template from pscis to add monitoirng fields as first test
...
# seems safer...
file_name <- paste0('form_monitor_', format(lubridate::now(), "%Y%m%d%H%m"))
...
# import our new columns
cols_monitor <- readr::read_csv('data/monitoring_params.csv') %>%
  # add a join field for simplicity now
  mutate(my_crossing_reference = 1805532)


# add the new columns to the field form

form_prep3 <- left_join(
  form_prep2,
  cols_monitor,
